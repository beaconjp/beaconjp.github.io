...
Вот пример ссылки с привязкой: http://alvarotrigo.com/fullPage/#it/2 (это URL, который вы увидите при доступе к данному разделу/слайду вручную)
Обратите внимание на то, что последняя часть URL заканчивается элементом `#it/2`.
...
```
В данном случае мы бы использовали URL `#it/slide3`, являющийся эквивалентным предыдущему - `#it/2`.
...
- Класс вида `fp-viewing-SECTION-SLIDE` добавляется к элементу `body`. (например: [`fp-viewing-it-0`](http://alvarotrigo.com/fullPage/#it)) Части `SECTION` и `SLIDE` будут являться привязками (или индексами, если привязки не предусмотрены) к текущему разделу и слайду.
...
```html
<ul id="myMenu">
	<li data-menuanchor="res" class="active"><a href="#res">Первый раздел</a></li>
	<li data-menuanchor="it"><a href="#it">Второй раздел</a></li>
	<li data-menuanchor="study"><a href="#study">Третий раздел</a></li>
	<li data-menuanchor="ad"><a href="#ad">Четвёртый раздел</a></li>
</ul>
```
```javascript
new fullpage('#fullpage', {
